...
bool Collision::intersect(const Ball& p, const Collectable& coll)
{
    sf::Vector2f distance = p.getPosition()-coll.getPosition();
    if(std::hypot(distance.x,distance.y) < (p.getSize() + coll.getSize()))
    {
        return true;
    }
    return false;
