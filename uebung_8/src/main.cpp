...
int main()
{
    ...
    objects.push_back(std::make_unique<Ball>(
                        sf::Vector2f(RNG::getInstance().getDirValue()*0.1,RNG::getInstance().getDirValue()*0.1),
    ...
    auto gui = GUI( sf::Vector2f(window.getSize().x / 2 - 40,window.getSize().y / 2),
                     sf::Vector2f(window.getSize().x / 2 + 40,window.getSize().y / 2));     
        
    auto observer = Observed(); // Create Observed
    
    auto c = CollectableManager();
    ...
    observer.addObserver(dynamic_cast<IObserver*>(objects.at(0).get()),EventType::PADDLE_ONE_COLLECTABLE);
    ...
    observer.addObserver(dynamic_cast<IObserver*>(objects.at(1).get()),EventType::PADDLE_TWO_COLLECTABLE);
    ...
    observer.addObserver(dynamic_cast<IObserver*>(objects.at(2).get()),EventType::BALL_Y_BORDER_COLLISION_LEFT);
     observer.addObserver(dynamic_cast<IObserver*>(objects.at(2).get()),EventType::BALL_Y_BORDER_COLLISION_RIGHT);  
     
    observer.addObserver(dynamic_cast<IObserver*>(&gui),EventType::BALL_Y_BORDER_COLLISION_RIGHT);  
    observer.addObserver(dynamic_cast<IObserver*>(&gui),EventType::BALL_Y_BORDER_COLLISION_LEFT); 
    ...
    while (window.isOpen())
    {
        ...
        for(auto e : c.checkCollisions(*dynamic_cast<Ball*>(objects.at(2).get())))
        {
        ...
        for(auto& i : borders)
        {
            if(Collision::intersect(*dynamic_cast<Ball*>(objects.at(2).get()),i))
            {
                ...
                {
                    if(dynamic_cast<Ball*>(objects.at(2).get())->getPosition().x < window.getSize().x *0.6)
                        observer.sendEvent(EventType::BALL_Y_BORDER_COLLISION_LEFT); 
                    else
                        observer.sendEvent(EventType::BALL_Y_BORDER_COLLISION_RIGHT); 
                }
            }
        }
